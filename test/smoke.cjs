...
// Materialize the factory with a stubbed require(). Nothing is rendered in
// this smoke test (only slot registration is asserted), so react/react-dom
// stubs are enough — no external react install is needed. (The previous
// approach required the react copy bundled inside the dsh CLI, which broke
// twice: the managed node dir migrated 22.22.2 -> 22.22.2-2, and dsh 0.1.3
// no longer ships a node-side react at all.)
const createElementStub = (type, props) => ({ type, props: props || {}, children: [] });
const reactStub = () => {
  const empty = () => {};
  return {
    useState: (v) => [typeof v === "function" ? v() : v, empty],
    useEffect: empty,
    useMemo: (fn) => fn(),
    useRef: (v) => ({ current: v }),
...
const requireFn = (spec) => {
  ...
  if (spec === "react/jsx-runtime") return { jsx: createElementStub, jsxs: createElementStub, Fragment: "Fragment" };
